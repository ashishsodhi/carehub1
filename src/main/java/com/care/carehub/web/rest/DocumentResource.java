...
/**
 * REST controller for managing {@link com.care.carehub.domain.Document}.
 */
@RestController
@RequestMapping("/api")
public class DocumentResource {
    ...
    public List<Document> getAllDocuments(@RequestParam(required = false) String filter) {
        if ("recipientitem-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where recipientItem is null");
            return documentService.findAllWhereRecipientItemIsNull();
        }
